...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        [InlineKeyboardButton("Добавить покупку", callback_data="buy")],
        [InlineKeyboardButton("Мой баланс", callback_data="mybalance")],
        [InlineKeyboardButton("Общий баланс", callback_data="balance")],
        [InlineKeyboardButton("Очистить долги", callback_data="reset")]
    ]
...
async def buy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    users=[u.replace("@","") for u in context.args[1:]]
    ...
    await update.message.reply_text(f"добавлено. доля {share:.2f}")
